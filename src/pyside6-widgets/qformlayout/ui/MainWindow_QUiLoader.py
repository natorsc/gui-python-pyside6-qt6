# -*- coding: utf-8 -*-
"""Python e Qt 6: PySide6 QFormLayout() com QUiLoader()."""
...
class MainWindow(QtCore.QObject):
    ...
    def __init__(self, application):
        ...
        self.ui.checkbox_show_pwd.stateChanged.connect(
            self.on_state_changed_show_pwd
        )

        self.ui.button_login.clicked.connect(self.on_button_clicked)
    ...
    def on_state_changed_show_pwd(self):
        echo_mode = self.ui.line_edit_password.echoMode()
        if echo_mode == QtWidgets.QLineEdit.Password:
            self.ui.line_edit_password.setEchoMode(QtWidgets.QLineEdit.Normal)
        else:
            self.ui.line_edit_password.setEchoMode(
                QtWidgets.QLineEdit.Password
            )

    def on_button_clicked(self, widget):
        print(f'Usuário: {self.ui.line_edit_user.text()}.')
        print(f'Senha: {self.ui.line_edit_password.text()}.')
